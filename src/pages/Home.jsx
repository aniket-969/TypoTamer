...
const Home = () => {
  ...
  const { visibility, setTypingProfile,mobileInput,setMobileInput,mobileInputVisible,setMobileInputVisible } = useTypingContext();
  ...
  return (
    ...
    <div className={` pb-2 sm:px-5 px-2 pt-6 ${visibility.settings || visibility.about ? "" : "h-screen overflow-hidden"} `}>
      ...
      {!visibility.leaderboard && !visibility.settings && !visibility.zen && !visibility.custom && !visibility.customWordsAndTime && !visibility.about && (
        <main className={`flex flex-col m-4 h-[84vh] py-5 px-2 items-center sm:gap-8 gap-4`}>

          <OptionSelector showOptions={showOptions} setShowOptions={setShowOptions} restart={restart} mobileInputVisible={mobileInputVisible}/>
          <CountedownTimer setShowOptions={setShowOptions} showOptions={showOptions} timeLeft={timeLeft} />
          <div ref={scrollRef} className={`relative [word-spacing:6px]  w-[100%] overflow-hidden flex items-center justify-center max-w-[1280px] ${showOptions ? "hidden" : "visible"} ${mobileInputVisible?"mt-0":"mt-5"} `}
          ...
          <MobileInput handleInput={handleInput} mobileInput={mobileInput} mobileInputVisible={mobileInputVisible} setMobileInputVisible={setMobileInputVisible} restart={restart} state={state}/>
          <VscDebugRestart className={`text-[1.2rem] restart ${mobileInputVisible?"hidden":""}`} onClick={() => restart()} />
          <Results 
            state={state} 
            errors={errors}
